...
import os
import re
import threading # Esencial para la carga asíncrona de portadas
import winreg 
import ctypes 
...
# --- CORRECCIÓN ERROR DE PERMISOS [Errno 13] ---
...
http_session.headers.update({'user-agent': 'AimpDiscordBridge/3.4'})
...
def cargar_configuracion():
    ...
    try:
        ...
        config['mostrar_caratula'] = bool(winreg.QueryValueEx(key, "mostrar_caratula")[0])
...
def guardar_configuracion():
    try:
        ...
        winreg.SetValueEx(key, "mostrar_caratula", 0, winreg.REG_DWORD, int(config_mostrar_caratula))
...
config_mostrar_caratula = cfg_cargada['mostrar_caratula']
...
def get_config_state():
    return (config_mostrar_pausa, config_mostrar_caratula, config_mostrar_linea, 
...
menu_ui = pystray.Menu(
    ...
    item('Estado de Reproducción', pystray.Menu(
        ...
        item('Mostrar Carátula del Álbum', set_toggle('config_mostrar_caratula'), checked=lambda i: config_mostrar_caratula),
...
def get_album_art_lastfm(artist, track, album_name):
    ...
    try:
        ...
        if 'track' in data and 'album' in data['track']:
            ...
            if img:
                suplente = img
                if data['track']['album'].get('title', "").lower().strip() == album_target or album_target in ["", "album", "sencillo/desconocido"]:
                    return img
    except: pass
    try:
    ...
    return suplente if suplente else "logo"

# --- CONTROLADOR DEL HILO ASÍNCRONO DE PORTADAS ---
forzar_update_portada = False

def buscar_portada_async(artista_raw, cancion_objetivo, album_objetivo):
    global portada_actual, forzar_update_portada
    
    url = get_album_art_lastfm(artista_raw, cancion_objetivo, album_objetivo)
    artista_principal = extraer_artista_principal(artista_raw)
    if url == "logo" and artista_principal != artista_raw:
        url = get_album_art_lastfm(artista_principal, cancion_objetivo, album_objetivo)
    
    # Validamos que el usuario no haya pasado a otra canción mientras la red respondía
    if cancion_objetivo == ultima_cancion_rpc:
        if url != "logo":
            try:
                # Verificación estricta de URL viva para evitar el "?" gris
                if http_session.head(url, timeout=3).status_code == 200:
                    portada_actual = url
                    forzar_update_portada = True
                    logger(f"Portada encontrada asíncronamente: {url}")
                    return
            except:
                pass
        # Si da error o es logo, se asienta en "logo" y actualiza fluidamente
        portada_actual = "logo"
        forzar_update_portada = True
# --------------------------------------------------
...
# Separamos el tracking: uno para detectar el cambio físico y otro para impactar a Discord
ultima_cancion_rpc = ""
        ...
        if info and estado_aimp != PlayBackState.Stopped:
            ...
            if estado_aimp == PlayBackState.Paused and not config_mostrar_pausa:
                if ultimo_estado != PlayBackState.Stopped or config_cambiada:
                    ...
                    ultima_cancion_en_discord = "" # Forzamos un refresco inmediato al volver a Play
                ultimo_estado_config = estado_config_actual
                time.sleep(2)
                continue

            cancion = info.get('title', 'Pista Desconocida')
            if not cancion: cancion = "Pista Desconocida"
            
            album = info.get('album', 'Sencillo/Desconocido')
            if not album: album = "Sencillo/Desconocido"

            artista_raw = info.get('artist', 'Artista Desconocido')
            if not artista_raw: artista_raw = "Artista Desconocido"

            posicion_actual = int(aimp.get_player_position() / 1000)
            duracion_total = int(info['duration'] / 1000)

            # --- DETECCIÓN INSTANTÁNEA (SIN CORTES) ---
            if cancion != ultima_cancion_rpc:
                ultima_cancion_rpc = cancion
                portada_actual = "logo" # Reseteo instantáneo a logo base (continuidad visual)
                logger(f"Nueva canción detectada: {cancion} - {artista_raw}")
                
                # Desplegamos el hilo en segundo plano para buscar la carátula sin congelar el programa
                threading.Thread(
                    target=buscar_portada_async, 
                    args=(artista_raw, cancion, album), 
                    daemon=True
                ).start()
            ...
            # Evaluamos si hay cambios (agregamos la señal del hilo: forzar_update_portada)
            if (cancion != ultima_cancion_en_discord or 
                ...
                img_enviar = portada_actual if config_mostrar_caratula else "logo"
                ...
                if config_icono_estado == "Solo Logo":
                    ...
                    s_img_enviar = v_small_default
                    s_txt_enviar = txt_estado
                ...
                try:
                    ...
                    # Sincronizamos estados tras el update exitoso
                    ultima_cancion_en_discord = cancion
                    ultimo_estado = estado_aimp
                    ultimo_update_discord = time.time()
                    ultimo_estado_config = estado_config_actual
                    forzar_update_portada = False # Consumimos el aviso del hilo
